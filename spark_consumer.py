...
class SparkHoeffdingEnsemble:
    ...
    def prediction_and_score(self, model, x_dict):
        ...
        if pred is None:
            # Cold-start fallback: choose class with highest probability, else normal.
            pred = max(proba, key=proba.get) if len(proba) > 0 else 0

        pred = int(pred)

        # Anomaly score is label-free. Higher means more suspicious.
        # If class 1 probability exists, use it directly.
        # Otherwise use uncertainty: 1 - confidence of predicted class.
        if 1 in proba:
            anomaly_score = float(proba.get(1, 0.0))
        else:
    ...
    def process_pdf(self, pdf):
        ...
        for idx, x in enumerate(X):
            ...
            for tree_id, model in enumerate(self.models):
                ...
                anomaly_signal = int(score >= 0.5)

                # Pseudo-label adaptation: production-compatible because it does not use y.
                # This is not as reliable as human-confirmed labels, but it avoids label leakage.
